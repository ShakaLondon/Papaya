        ...
        <Navbar bg="light" expand="lg" className="px-0 py-0" id="bus-profile-header" fixed="top">
            ...
            <Container fluid className="d-flex fullopacity" id="profile-container"  style={{ paddingTop: `${ colorChangeState ? '110px' : '150px' }`}}>
                ...
                <FontAwesomeIcon icon={faChevronLeft} className="fa-sm text-white"/>
                  { (category?.categoryLevel == 1) ? <h6 className="text-white mb-0 mx-2">All Categories</h6> : <h6 className="text-white mb-0 mx-2">{category.parentCategory[0]?.name}</h6>}
                </div>
                <h1 className="text-white mb-0">{category?.name}</h1>
                <div className="d-flex align-items-center">
                  <h6 className="text-white mb-0 align-self-center">{`Compare companies in the ${category?.name} category`}</h6>
