...
function StarRating({ currentNum, relocate, current, changeable, fontSize, website, selected }) {
    ...
    const changeHoverRatingEnd = (e) => {
      ...
      if (currentRating > 0) {
        ...
        const halfValue = remainder < 0.3 ? 0 :
        remainder < 0.8 ? 0.5 :
        1
        ...
        return (
          <div className="d-flex">
            {[1, 2, 3, 4, 5].map((value) => (
                <Star 
                 key={value} 
                 filled={ halfValue === 1 ? value <= (currentRating + 1) : value <= currentRating }
                 ratingValue={currentRating}
                 number={value}
                 hoverValue={0}
                 onClick={() => { selected(value); changeRatingSet(value);}} 
    ...
    <div className="d-flex">
      {[1, 2, 3, 4, 5].map((value) => (
          <Star 
           ...
           onClick={() => {selected(value); changeRating(value, website);}} 
           onMouseOver={(e) => changeHoverRating(e, value)}
           onMouseOut={() => changeHoverRatingEnd()}
           set={0}
           className="star"
